...
client.defineJob({
    id: Jobs.TRANSACTIONS_MANUAL_SYNC_V2,
    name: "Transactions - Manual Sync V2",
    version: "0.0.3",
    trigger: eventTrigger({
        name: Events.TRANSACTIONS_MANUAL_SYNC_V2,
        schema: z.object({
        ...
        const syncPromises = bankAccounts?.map(async (account) => {
            try {
                ...
                const fetchedTransactions = await engine.transactions.list({
                    ...
                    accountType: account.type as "depository" | "credit" | "other_asset" | "loan" | "other_liability" | undefined,
                ...
                await uniqueLog(
                    io,
                    "error",
                    `Error syncing account: ${JSON.stringify(syncError)}`,
                    {
                        accountId: account.id,
                        error: syncError,
                    }
                ...
                if (syncError instanceof FinancialEngine.APIError) {
