...
export const InteractiveFeatures: StoryFn<InteractiveBarChartProps> = (args: any) => {
    const [chartData, setChartData] = React.useState(generateChartData(30, ['desktop', 'mobile']));

    const handleDrilldown = (startDate: string, endDate: string) => {
        const newData = generateChartData(15, ['desktop', 'mobile'], new Date(startDate), new Date(endDate));
        setChartData(newData);
        alert(`Drilled down from ${new Date(startDate).toLocaleString()} to ${new Date(endDate).toLocaleString()}`);
    };
