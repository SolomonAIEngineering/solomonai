...
export function ZoomableChartWithDrilldown({
    ...
    const handleZoom = (
        ...
        e.preventDefault();
        if (originalData.length === 0 || !originalData.length || !chartRef.current) return;
        ...
        <Card className="w-full h-full">
            ...
            {selectedDataPoint && (
                <DrilldownDialog
                    ...
                    surroundingData={selectedDataPoint ? getSurroundingData(selectedDataPoint, data) : []}
    ...
    if (active && payload && payload.length) {
        ...
        return (
            <div className="bg-white p-4 border rounded shadow">
                ...
                <p className="text-sm text-gray-500 mt-2">Click for detailed insights</p>
            </div>
        );
    }
    return null;
...
const DrilldownDialog: React.FC<DrilldownDialogProps> = ({ isOpen, onClose, dataPoint, surroundingData, onDrilldown }) => {
        ...
        <Dialog open={isOpen} onOpenChange={onClose}>
            <DialogContent className="sm:max-w-[600px] p-[2%]">
                ...
                <div className="mt-4 space-y-4">
                    <div>
                        <h3 className="text-lg font-semibold">Transaction Count: {dataPoint.events}</h3>
                        <p>Time of Day: {format(date, 'p')}</p>
                        <p>Day of Week: {format(date, 'EEEE')}</p>
                    </div>
                    <div>
                        <h4 className="font-semibold">Trend Analysis:</h4>
                        <p>This is a {getActivityLevel(dataPoint.events).toLowerCase()} activity period.</p>
                        <p>Current trend: <span className="font-medium">{trend}</span></p>
                        <p>Average transactions in this period: <span className="font-medium">{average.toFixed(2)}</span></p>
                        <p>
                            This day is {dataPoint.events > average ? 'above' : 'below'} average by{' '}
                            <span className="font-medium">{Math.abs(dataPoint.events - average).toFixed(2)}</span> transactions.
                        </p>
                    </div>
                    <div>
                        <h4 className="font-semibold">Comparative Analysis:</h4>
                        <p>Average transactions on {format(date, 'EEEE')}s:
                            <span className="font-medium"> {averageForSimilarDays}</span>
                        </p>
                        {averageForSimilarDays !== 'No data' && (
                            <p>
                                This {format(date, 'EEEE')} is {dataPoint.events > parseFloat(averageForSimilarDays) ? 'more' : 'less'} active than usual.
                            </p>
                        )}
                    </div>
                    <div>
                        ...
                        <ul className="list-disc pl-5">
                            {getPersonalFinanceRecommendations(dataPoint.events).map((rec, index) => (
                                <li key={index}>{rec}</li>
                            ))}
                        </ul>
                    </div>
