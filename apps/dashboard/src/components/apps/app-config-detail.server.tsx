                ...
                <Tabs defaultValue="overview">
                    ...
                    <TabsContent value="overview">
                        <div className="flex flex-col gap-6 p-[2.5%]">
                            ...
                            {config.supported_features && config.supported_features.length > 0 && (
                                <section>
                        ...
                        <div className="flex flex-col gap-6 p-[2.5%]">
                            {config.settings.map((setting, index) => (
                                <div key={index} className="flex flex-col gap-3">
                                    ...
                                    {setting.type === "text" && (
                                        <Input id={`setting-${index}`} type="text" placeholder={setting.description} />
                                    )}
                                    {setting.type === "number" && (
                                        <Input id={`setting-${index}`} type="number" placeholder={setting.description} />
                                    )}
                                    {setting.type === "select" && (
                                        <Select>
                    ...
                    <TabsContent value="permissions">
                        <div className="flex flex-col gap-6 p-[2.5%]">
                            {Object.entries(config.user_permissions || {}).map(([role, permissions]) => (
                                <div key={role}>
                    ...
                    <TabsContent value="advanced">
                        <div className="flex flex-col gap-6 p-[2.5%]">
                            ...
                            <div className="flex justify-between items-center">
                                ...
                                <span className={config.sync_status === 'active' ? 'text-green-500' : 'text-red-500'}>
