...
export const addAppToAccountAction = authActionClient
    .schema(
        z.object({
            ...
            sync_status: z.enum(["active", "paused", "error"]).optional().default("active"),
            tags: z.array(z.string()).nullable(),
            user_permissions: z.record(z.string(), z.array(z.string())).optional(),
            version: z.string().optional(),
            webhook_url: z.string().url().optional().default("https://gateway.solomonai-platform.com/webhook"),
        }),
    )
    .metadata({
    ...
    .action(
        async ({ parsedInput, ctx: { user, supabase } }) => {
            const { 
                app_id, 
                app_name, 
                category,
