...
-- Function to get recurring transactions with flexible filtering
CREATE OR REPLACE FUNCTION get_recurring_transactions(
  p_stream_id TEXT DEFAULT NULL,
  p_account_id TEXT DEFAULT NULL,
  p_merchant_name TEXT DEFAULT NULL,
  p_start_date DATE DEFAULT NULL,
  p_last_transaction_date DATE DEFAULT NULL,
...
RETURNS TABLE (
  ...
  last_user_modified_datetime TIMESTAMP
) AS $$
BEGIN
  RETURN QUERY
  SELECT rt.*
  ...
  WHERE
    (p_stream_id IS NULL OR rt.stream_id = p_stream_id)
    AND (p_account_id IS NULL OR rt.account_id = p_account_id)
    AND (p_merchant_name IS NULL OR rt.merchant_name ILIKE '%' || p_merchant_name || '%')
    AND (p_start_date IS NULL OR rt.first_date >= p_start_date)
    AND (p_last_transaction_date IS NULL OR rt.last_date <= p_last_transaction_date)
  ORDER BY rt.last_date DESC, rt.first_date DESC
  LIMIT p_limit
  OFFSET p_offset;
END;
$$ LANGUAGE plpgsql;
